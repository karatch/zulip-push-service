...
class ZulipTelegramBridge:
    def __init__(self, tg_token: str, zulip_site: str, loop: asyncio.AbstractEventLoop, zuliprc_path: Path):
        ...
        self.session = None
        self.semaphore = asyncio.Semaphore(20)  # семафор на 20 сокетов
    ...
    async def send_telegram_push(self, tg_chat_id: int, stream_name: str, topic: str, sender_name: str,
        ...
        # Конвертирую сырой Markdown для корректного отображения внутри Telegram HTML parse_mode
        ...
        async with self.semaphore:
            try:
                async with self.session.post(url, json=payload, timeout=3) as response:
    ...
    def process_event(self, event: dict) -> None:
        ...
        # контент content_raw для чистого текста без HTML-тегов
            ...
            if tg_id:
                ...
                self.loop.call_soon_threadsafe(
                    lambda t=tg_id, sn=stream_name: asyncio.create_task(
                        self.send_telegram_push(int(t), sn, topic, sender_name, content, msg_url)
                    )
                )
    ...
    async def start(self, session):
        ...
        async def safe_listener_loop():
            while True:
                # выделенный пул потоков с Daemon-воркерами
